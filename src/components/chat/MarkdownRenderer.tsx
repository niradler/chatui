...
const MermaidDiagram: React.FC<{ chart: string; darkMode?: boolean }> = ({ chart, darkMode }) => {
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      try {
        // Configure mermaid theme based on dark mode
        mermaid.initialize({
      ...
      <ReactMarkdown
        ...
        components={{
          code({ node, className, children, ...props }) {
            ...
            if (language === 'mermaid') {
              return <MermaidDiagram chart={String(children).trim()} darkMode={darkMode} />;
            }
          ...
          td({ children, ...props }) {
            return (
              <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700" {...props}>
                {children}
              </td>
            );
          },
          ...
          h1({ children, ...props }) {
            return <h1 className="text-2xl font-bold mt-6 mb-4 text-gray-900 dark:text-gray-100" {...props}>{children}</h1>;
          },
          
          h2({ children, ...props }) {
            return <h2 className="text-xl font-semibold mt-5 mb-3 text-gray-900 dark:text-gray-100" {...props}>{children}</h2>;
          },
          
          h3({ children, ...props }) {
            return <h3 className="text-lg font-medium mt-4 mb-2 text-gray-900 dark:text-gray-100" {...props}>{children}</h3>;
          },
